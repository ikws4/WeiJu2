...
class XposedLib extends TwoArgFunction {
    ...
    static final class hook extends OneArgFunction {
        ...
        public LuaValue call(LuaValue arg) {
            ...
            var classLoader = (ClassLoader) table.get("classloader").checkuserdata();
            ...
            if (method.isnil()) {
                unhook = XposedHelpers.findAndHookConstructor(clazz, classLoader, _params);
            } else {
               unhook = XposedHelpers.findAndHookMethod(clazz, classLoader, method.checkjstring(), _params);
            }
