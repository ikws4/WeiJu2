...
/**
 * Copyed from Android source code, but with fuzzy filter
 */
  ...
  private class FuzzyFilter extends Filter {
    ...
    protected FilterResults performFiltering(CharSequence filterStr) {
      ...
      if (filterStr == null || filterStr.length() == 0) {
        ...
        final String constrains = filterStr.toString();
    ...
    private boolean match(String s, String t) {
      ...
      for (int i = 0; i < t.length(); i++) {
        ...
        while (j < s.length() && Character.toLowerCase(s.charAt(j)) != b) {
          j++;
        }
